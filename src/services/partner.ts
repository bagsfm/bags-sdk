...
import {
	...
	fetchBagsFeeShareV2PartnerConfig,
} from '../utils/fee-share-v2/partner-config';
...
export class PartnerService extends BaseService {
	...
	async getPartnerConfig(wallet: PublicKey): Promise<DecodedPartnerConfig> {
		...
		const partnerConfigData = await fetchBagsFeeShareV2PartnerConfig(
			partnerConfig,
			this.connection,
			this.commitment,
			this.bagsFeeShareV2Coder
		);
	...
	async getPartnerConfigCreationTransaction(
		partner: PublicKey
	): Promise<{ transaction: VersionedTransaction; blockhash: BlockhashWithExpiryBlockHeight }> {
		const response = await this.bagsApiClient.post<{ transaction: TransactionWithBlockhash }>(
	...
	async getPartnerConfigClaimStats(partner: PublicKey): Promise<PartnerConfigClaimStatsResponse> {
		const response = await this.bagsApiClient.get<PartnerConfigClaimStatsResponse>(
	 ...
	 */
	async getPartnerConfigClaimTransactions(
		partner: PublicKey
	): Promise<Array<{ transaction: VersionedTransaction; blockhash: BlockhashWithExpiryBlockHeight }>> {
		const response = await this.bagsApiClient.post<{ transactions: Array<TransactionWithBlockhash> }>(
