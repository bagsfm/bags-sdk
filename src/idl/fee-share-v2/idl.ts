...
export type BagsFeeShare = {
	...
	instructions: [
		...
		{
			name: 'claimUserVault';
			docs: ['Claim user fees from their vault as native SOL (closes vault ATA)'];
			discriminator: [216, 180, 29, 188, 12, 5, 43, 8];
			accounts: [
				{
					name: 'user';
					docs: ["The claiming user; must match the vault PDA's user seed"];
					writable: true;
					signer: true;
				},
				{
					name: 'userFeeVault';
					docs: ['Vault PDA — authority for the vault ATA; persists across claims for GPA discoverability.'];
					pda: {
						seeds: [
							{
								kind: 'const';
								value: [117, 115, 101, 114, 95, 102, 101, 101, 95, 118, 97, 117, 108, 116];
							},
							{
								kind: 'account';
								path: 'user';
				...
				},
				{
					...
					docs: ["Vault's WSOL ATA (closed by this instruction; all lamports go to user)"];
					writable: true;
					pda: {
						seeds: [
							{
		...
		{
			name: 'forceClaimUser';
			docs: ["Force claim user fees (admin only); delivers WSOL to user's ATA"];
		...
		},
		{
			...
			docs: ['Force claim user fees into a program-controlled vault (admin or manager).', 'User can later claim from the vault via `claim_user_vault`.'];
			discriminator: [9, 21, 27, 21, 209, 150, 12, 142];
			accounts: [
				{
					name: 'payer';
					docs: ['Payer funding vault account and ATA initialization'];
					writable: true;
					signer: true;
				},
				{
					name: 'authority';
					docs: ['Admin or manager signer authorizing the force claim to vault'];
					signer: true;
				},
				{
					name: 'programConfig';
					docs: ['Singleton program config PDA (holds admin pubkey for auth check)'];
					pda: {
						seeds: [
							{
								kind: 'const';
				...
				{
					name: 'userVaultQuoteAta';
					docs: ["Vault's WSOL ATA (destination; init-if-needed, authority = user_fee_vault PDA)"];
					writable: true;
					pda: {
						seeds: [
							{
		...
		},
		{
			...
			docs: ['Force claim user fees as native SOL (admin only); unwraps WSOL and sends SOL to user'];
			discriminator: [78, 63, 180, 4, 151, 16, 107, 241];
			accounts: [
				{
					name: 'admin';
					docs: ['Admin signer authorizing force claim'];
				...
				{
					name: 'user';
					docs: ['Claimer user (no signature required). Receives native SOL when escrow is closed.'];
					writable: true;
				},
				{
					name: 'feeShareConfig';
				...
				},
				{
					...
					pda: {
						seeds: [
							{
								kind: 'const';
								value: [102, 111, 114, 99, 101, 95, 99, 108, 97, 105, 109, 95, 101, 115, 99, 114, 111, 119];
							},
							{
								kind: 'account';
								path: 'user';
		...
		},
		{
			...
			docs: ['Manager transfers their managerial position (requires both old and new manager signatures)'];
			discriminator: [141, 74, 17, 174, 124, 11, 170, 227];
			accounts: [
				{
					name: 'payer';
					docs: ['Payer funding the transfer'];
					writable: true;
					signer: true;
				},
				{
					name: 'manager';
					docs: ['Current manager signer authorizing the transfer'];
					writable: true;
					signer: true;
				},
				{
				...
				{
					name: 'feeShareConfig';
					docs: ['Fee share config (PDA) whose manager is being transferred'];
					writable: true;
					pda: {
						seeds: [
							{
				...
				{
					name: 'feeShareAuthority';
					docs: ['Fee share authority (PDA) - needed for event emission and validation'];
					pda: {
						seeds: [
							{
								kind: 'const';
		...
		},
		{
			...
			docs: ['Manager update fee config (claimers + bps)'];
			discriminator: [84, 56, 125, 101, 210, 214, 195, 197];
			accounts: [
				{
					name: 'payer';
					docs: ['Payer funding the update'];
					writable: true;
					signer: true;
				},
				{
					name: 'manager';
					docs: ['Manager signer authorizing the update'];
					writable: true;
					signer: true;
				},
				{
		...
		},
		{
			...
			docs: ['Manager waives their managerial position (sets manager to Pubkey::default)'];
			discriminator: [105, 51, 140, 114, 254, 160, 173, 172];
			accounts: [
				{
					name: 'payer';
					docs: ['Payer funding the operation'];
					writable: true;
					signer: true;
				},
				{
					name: 'manager';
					docs: ['Current manager signer waiving their role'];
					writable: true;
					signer: true;
				},
				{
					name: 'feeShareConfig';
					docs: ['Fee share config (PDA) whose manager is being waived'];
					writable: true;
					pda: {
						seeds: [
							{
		...
		},
		{
			...
			docs: ['Update the manager on an existing fee config'];
			discriminator: [196, 8, 168, 199, 166, 4, 77, 212];
			accounts: [
				{
					name: 'payer';
					docs: ['Payer funding the update'];
				...
				{
					name: 'feeShareConfig';
					docs: ['Fee share config (PDA) whose manager is being updated'];
					writable: true;
					pda: {
						seeds: [
							{
		...
		},
		{
			...
			docs: ['Update partner on an existing fee config'];
			discriminator: [175, 155, 78, 199, 239, 2, 95, 77];
			accounts: [
				{
					name: 'admin';
					docs: ['Admin signer authorizing the update'];
				...
				{
					name: 'feeShareConfig';
					docs: ['Fee share config (PDA) whose partner is being updated'];
					writable: true;
					pda: {
						seeds: [
							{
				...
				{
					name: 'feeShareAuthority';
					docs: ['Fee share authority (PDA) - needed for event emission'];
					pda: {
						seeds: [
							{
								kind: 'const';
				...
				{
					name: 'partnerConfig';
					docs: ['Optional new partner config (PDA); must belong to `partner` if provided'];
					optional: true;
					pda: {
						seeds: [
							{
				...
				{
					name: 'partner';
					docs: ['Optional new partner wallet; must match partner_config if provided'];
					optional: true;
				},
				{
					name: 'systemProgram';
		...
		},
		{
			...
			docs: ['**@deprecated: use `update_partner_fee_collection` instead (2.1.0)**'];
		...
		},
		{
			...
			docs: ['Updates:', '* partner fee collection mode', '* partner fee collection platform bps', '* partner fee collection claimers bps'];
			discriminator: [155, 56, 159, 217, 130, 217, 252, 72];
			accounts: [
				{
					name: 'admin';
					docs: ['Admin signer authorizing update'];
				...
				{
					name: 'partnerConfig';
					docs: ['Partner config (PDA) to update; must belong to `partner`'];
					writable: true;
					pda: {
						seeds: [
							{
				...
				{
					name: 'systemProgram';
					docs: ['System program (future-proofing)'];
					address: '11111111111111111111111111111111';
				},
				{
					name: 'eventAuthority';
	...
	accounts: [
		...
		{
			name: 'userFeeVault';
			discriminator: [117, 20, 152, 251, 163, 18, 193, 192];
		},
	];
	events: [
		{
		...
		},
		{
			...
			discriminator: [195, 23, 211, 154, 30, 157, 169, 52];
		},
		{
			name: 'bagsFeeShareUserVaultClaimEvent';
			discriminator: [58, 0, 147, 243, 250, 139, 221, 102];
		},
		{
		...
		},
		{
			...
			discriminator: [119, 106, 2, 17, 241, 76, 128, 184];
		},
		{
		...
		},
		{
			...
			discriminator: [44, 79, 22, 247, 183, 216, 168, 183];
		},
		{
			...
			type: {
				...
				fields: [
					...
					},
					{
						...
						docs: ['Who force-executed the claim'];
						type: {
							option: 'pubkey';
						};
					},
		...
		},
		{
			...
			type: {
				...
				fields: [
					...
					{
						name: 'user';
						docs: ['The user who claimed from their vault'];
						type: 'pubkey';
					},
					{
						name: 'baseMint';
						docs: ['Mint of the base token (identifies the fee config pair)'];
						type: 'pubkey';
					},
					{
						name: 'quoteMint';
						docs: ['Mint of the quote token (WSOL)'];
						type: 'pubkey';
					},
					{
						name: 'claimed';
						docs: ['Amount of native SOL delivered to user (WSOL balance + rent from closed ATA)'];
						type: 'u64';
					},
				];
			};
		...
		},
		{
			...
			type: {
				...
				fields: [
					...
					{
						name: 'admin';
						docs: ['Admin or manager that triggered the update'];
						type: 'pubkey';
					},
					{
						name: 'payer';
					...
					{
						name: 'padding';
						docs: ['Reserved for future event fields'];
						type: {
							array: ['u64', 4];
						};
					},
		...
		},
		{
			...
			type: {
				...
				fields: [
					...
					{
						name: 'manager';
						docs: ['Optional manager pubkey (defaults to Pubkey::default() if no manager is set)'];
						type: 'pubkey';
					},
					{
						name: 'padding0';
			...
			type: {
				...
				fields: [
					...
					},
					{
						...
						docs: ['Partner fee collection mode (stored as u8; see [`PartnerFeeCollectionMode`])', '', 'Defaults to 0 (CollectFromClaimers).'];
						type: 'u8';
					},
					{
						name: 'feeCollectionPlatformBps';
						docs: ['Partner fee collection bps from platform share (used for CollectFromPlatform and CollectFromAll)', '', 'Defaults to 0.'];
						type: 'u16';
					},
					{
						name: 'padding1';
					...
					{
						name: 'bps';
						docs: ['Partner share in basis points (before distribution to claimers)', '', 'Updated name: **fee_collection_claimers_bps**'];
						type: 'u16';
					},
					{
						name: 'padding0';
						docs: ['Padding reserved for future use (8-byte aligned; kept for backwards-compatible account size)'];
						type: {
							array: ['u64', 5];
						};
					},
		...
		},
		{
			...
			type: {
				...
				fields: [
					...
					{
						name: 'feeCollectionMode';
						docs: ['Partner fee collection mode'];
						type: 'u8';
					},
					{
						name: 'feeCollectionPlatformBps';
						docs: ['Partner fee collection platform bps'];
						type: 'u16';
					},
					{
						name: 'feeCollectionClaimersBps';
						docs: ['Partner fee collection claimers bps'];
						type: 'u16';
					},
					{
						name: 'padding0';
		...
		},
		{
			...
			type: {
				...
				fields: [
					...
					{
						name: 'feeCollectionPlatformBps';
						docs: ['Partner fee collection bps from the platform pool.'];
						type: 'u16';
					},
					{
						name: 'feeCollectionClaimersBps';
		...
		},
		{
			...
			type: {
				...
				fields: [
					{
						name: 'user';
						docs: ['The user (claimer) this vault belongs to'];
						type: 'pubkey';
					},
					{
						name: 'baseMint';
						docs: ['Base mint of the fee share config'];
						type: 'pubkey';
					},
					{
						name: 'quoteMint';
